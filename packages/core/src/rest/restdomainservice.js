 ...
 * @WebdaModda
 */
export class RESTDomainService extends ModelsOperationsService {
    /**
    ...
    resolve() {
        var _a;
        (_a = this.parameters).exposeOpenAPI ?? (_a.exposeOpenAPI = useCore().isDebug());
        super.resolve();
     ...
     * @returns
     */
    handleModel(model, name, context) {
        ...
        let openapi = {
            [this.parameters.queryMethod.toLowerCase()]: {
                ...
                requestBody: this.parameters.queryMethod === "GET"
                    ? undefined
                    : {
                        content: {
                            "application/json": {
                ...
                parameters: this.parameters.queryMethod === "GET"
                    ? [
                        {
                            name: "q",
                            in: "query",
        ...
        if (!actionsName.includes("query")) {
            this.addRoute(`${prefix}${this.parameters.queryMethod === "GET" ? "{?q?}" : ""}`, [this.parameters.queryMethod], async (context) => {
                let queryString = "";
        ...
        if (!actionsName.includes("create")) {
            this.addRoute(`${prefix}`, ["POST"], async (context) => {
        ...
        if (!actionsName.includes("delete")) {
            this.addRoute(`${prefix}/{uuid}`, ["DELETE"], (context) => callOperation(context, `${shortId}.Delete`), openapi);
        }
        const openapiInfo = {
            tags: [name],
        ...
        if (!actionsName.includes("update")) {
            this.addRoute(`${prefix}/{uuid}`, ["PUT", "PATCH"], (context) => {
                if (context.getHttpContext().getMethod() === "PUT") {
        ...
        if (!actionsName.includes("get")) {
            this.addRoute(`${prefix}/{uuid}`, ["GET"], (context) => callOperation(context, `${shortId}.Get`), openapi);
        }
        // Add all actions
        ...
        Object.keys(actions).forEach(actionName => {
            ...
            if (hasSchema(`${Identifier}.${actionName}.output`)) {
                Object.keys(openapi)
                    ...
                    .forEach(k => {
                    var _a, _b;
                    (_a = openapi[k]).responses ?? (_a.responses = {});
                    (_b = openapi[k].responses)["200"] ?? (_b["200"] = {});
            ...
            this.addRoute(action.global ? `${prefix}/${actionName}` : `${prefix}/{uuid}/${actionName}`, action.methods || ["PUT"], async (context) => {
        ...
        */
        (relations.binaries || []).forEach(binary => {
            const modelInjector = async (context) => {
            ...
            const modelInjectorChallenge = async (context) => {
                const attribute = binary.attribute.substring(0, 1).toUpperCase() + binary.attribute.substring(1);
                return callOperation(context, `${shortId}.${attribute}.AttachChallenge`);
            };
            const modelInjectorGet = async (context) => {
                const attribute = binary.attribute.substring(0, 1).toUpperCase() + binary.attribute.substring(1);
                if (binary.cardinality === "MANY") {
                    context.getParameters().index = parseInt(context.parameter("index"));
                }
                return callOperation(context, `${shortId}.${attribute}.Get${context.getHttpContext().getUrl().endsWith("url") ? "Url" : ""}`);
            };
            openapi = {
                put: {
                    tags: [name],
     ...
     */
    async openapi(ctx) {
        this.openapiContent ?? (this.openapiContent = SWAGGER_HTML.replace(/\{\{VERSION}}/g, this.parameters.swaggerVersion).replace("{{OPENAPI}}", JSON.stringify(useRouter().exportOpenAPI(true))));
        ctx.write(this.openapiContent);
    }
}
//# sourceMappingURL=restdomainservice.js.map