...
class ApiKeyTest extends WebdaTest {
  ...
  toHawkCredentials() {
    ...
    assert.strictEqual(
      ...
      "sha256",
      "Should default to sha256 algorithm"
    );
    assert.strictEqual(
      credentials.key,
      "bouzouf",
      "Key should equal to hidden value __secret"
    );
  }
  ...
  toHawkCredentialsWithAlgorithm() {
    this.apikey.load(
      { __secret: "bouzouf", uuid: "plop", algorithm: "md5" },
      true
    );
    const credentials = this.apikey.toHawkCredentials();
    assert.strictEqual(
  ...
  checkOrigin() {
    ...
    this.getExecutor(
      ...
      "/origins",
      {},
      { origin: "https://test.webda.io/" }
    );
    assert.ok(
      this.apikey.checkOrigin(this.context.getHttpContext()),
    ...
    assert.ok(
      !this.apikey.canRequest(this.context.getHttpContext()),
      "localhost https should be refused"
    );
  }
  ...
  checkWhitelist() {
    ...
    this.getExecutor(
      ...
      "test.webda.io",
      "PUT",
      "/path/to/the/valhalla"
    );
    assert.ok(
      this.apikey.canRequest(
  ...
  canRequestNoOrigin() {
    ...
    assert.ok(
      this.apikey.canRequest(this.context.getHttpContext()),
      "valhalla PUT should be granted"
    );
  }
  ...
  canRequestNoPermissions() {
    this.apikey.load(
      { ...KEY, origins: undefined, permissions: undefined },
      true
    );
