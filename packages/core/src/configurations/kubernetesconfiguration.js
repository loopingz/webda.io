...
export class KubernetesConfigurationParameters extends ConfigurationServiceParameters {
    load(params) {
        super.load(params);
        if (Object.keys(params.default?.services || {}).length ||
            Object.keys(params.default?.application || {}).length ||
            Object.keys(params.default?.parameters || {}).length) {
            throw new WebdaError.CodeError("KUBE_CONFIGURATION_NO_DEFAULT", `KubernetesConfigurationService does not support default configuration ${params.default}`);
        }
...
export class KubernetesConfigurationService extends ConfigurationService {
    ...
    getFilePath(id) {
        var _a;
        (_a = this.sourcePaths)[id] ?? (_a[id] = useApplication().getPath(id));
        if (!fs.existsSync(this.sourcePaths[id])) {
            throw new WebdaError.CodeError("KUBE_CONFIGURATION_SOURCE_MISSING", `Kubernetes configuration source not found: ${this.sourcePaths[id]}`);
        }
     ...
     * @returns
     */
    canTriggerConfiguration(id, callback) {
        var _a;
    ...
    async getConfiguration(id) {
        ...
        const found = fs
            ...
            .pop();
        if (!found) {
            throw new WebdaError.CodeError("KUBE_CONFIGURATION_SOURCE_INVALID", `Kubernetes configuration source not valid: ${this.sourcePaths[id]}`);
        }
        return FileUtils.load(path.join(this.sourcePaths[id], found));
    }
}
//# sourceMappingURL=kubernetesconfiguration.js.map