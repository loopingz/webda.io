...
export class DomainServiceParameters extends ServiceParameters {
    load(params = {}) {
        super.load(params);
        // Init default here
        this.operations ?? (this.operations = true);
        this.nameTransfomer ?? (this.nameTransfomer = "camelCase");
        this.queryMethod ?? (this.queryMethod = "PUT");
...
export class DomainService extends Service {
     ...
     * @returns
     */
    walkModel(model, name, depth = 0, modelContext = {}) {
    ...
    async getModel(context) {
        const { model } = context.getExtension("operationContext");
     ...
     * @param context
     */
    async modelAction(context) {
        const { model, action } = context.getExtension("operationContext");
        if (!action.global) {
    ...
    initOperations() {
        ...
        const app = (this.app = useApplication());
        ...
        for (const modelKey in models) {
            ...
            this.addBinaryOperations(model, Metadata, shortId);
        }
    }
    addBinaryOperations(model, Metadata, name) {
     ...
     * @param context
     */
    async binaryChallenge(context) {
        const body = await context.getInput();
        const { model, binaryStore, binary } = context.getExtension("operationContext");
     ...
     * @returns
     */
    checkBinaryAlreadyLinked(property, hash) {
     ...
     * @param context
     */
    async binaryGet(context) {
        const { model, returnUrl, binaryStore, binary } = context.getExtension("operationContext");
        ...
        if (url === null) {
            if (returnUrl && context instanceof WebContext) {
                ...
                const readStream = await binaryStore.get(file);
    ...
    async binaryAction(context) {
        const { model, binaryStore, binary, action } = context.getExtension("operationContext");
        ...
        if (action === "delete") {
            ...
            const metadata = await context.getInput();
 ...
 * @WebdaModda
 */
export class ModelsOperationsService extends DomainService {
    /**
     * Default domain
     */
    loadParameters(params) {
        return new DomainServiceParameters().load(params);
    }
    /**
     * Do nothing here
     */
