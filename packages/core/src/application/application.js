...
import { isModelClass } from "@webda/models";
...
let Application = (() => {
    ...
    return _a = class Application {
             ...
             * @param {Logger} logger
             */
            constructor(file, logger = undefined) {
                /**
                 * Get Application root path
                 */
                this.applicationPath = __runInitializers(this, _instanceExtraInitializers);
                /**
                ...
                if (!existsSync(file)) {
                    throw new WebdaError.CodeError("NO_WEBDA_FOLDER", `Not a webda application folder or webda.config.(ya?ml|jsonc?) file: unexisting ${file}`);
                }
            ...
            async loadConfiguration(file) {
                ...
                if (file && !existsSync(file)) {
                    throw new WebdaError.CodeError("NO_WEBDA_FOLDER", `Not a webda application folder or webda.config.jsonc or webda.config.json file: ${file}`);
                }
                try {
                    this.baseConfiguration ?? (this.baseConfiguration = file ? FileUtils.load(file) : {});
             ...
             */
            getPackageWebda() {
                return (this.baseConfiguration.cachedModules.project?.webda || {
                    namespace: "Webda"
                });
            }
            /**
            ...
            getWebdaObject(section, name) {
                ...
                if (!this[section][objectName]) {
                    ...
                    throw Error(`Undefined ${section.substring(0, section.length - 1)} ${objectName} (${Object.keys(this[section]).join(", ")})`);
                }
             ...
             */
            getGitInformation(_packageName, _version) {
             ...
             * @returns
             */
            getConfiguration(_deployment = undefined) {
             ...
             * @param info
             */
            async importFile(info, withExport = true) {
                if (info.startsWith(".")) {
             ...
             */
            async loadModule(module, parent = this.applicationPath) {
                if (!module) {
                    return;
                }
                const info = module;
                const sectionLoader = async (section) => {
                    var _b;
                    for (const key in info[section]) {
                        (_b = this[section])[key] ?? (_b[key] = await this.importFile(join(parent, info[section][key].Import)));
                        if (!this[section][key]) {
                            this.log("ERROR", `Cannot load ${section.substring(0, section.length - 1)} ${key} from ${join(parent, info[section][key].Import)}`);
                            continue;
                        }
                        if (section === "beans" || section === "moddas") {
                            ...
                            if (!configurationClass) {
                                this.log("ERROR", `Cannot load configuration for ${section.substring(0, section.length - 1)} ${key} from ${join(parent, info[section][key].Configuration)}`);
                                continue;
                            }
                            this[section][key].filterParameters = (params = {}) => {
                            ...
                            this[section][key].createConfiguration = (params = {}) => {
                ...
                await Promise.all([
                    ...
                    ...Object.keys(info.beans || {})
                        ...
                        if (this.baseConfiguration?.parameters?.ignoreBeans &&
        ...
        (() => {
            ...
            _load_decorators = [ApplicationState({ start: "loading", end: "ready" }), InstanceCache()];
