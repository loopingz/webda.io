...
export class FargateDeployer extends DockerMixIn(AWSDeployer) {
  ...
  async deploy(args) {
    ...
    this.resources.repositoryNamespace =
      this.resources.repositoryNamespace === undefined
        ? this.resources.serviceName
        : this.resources.repositoryNamespace;
    this.resources.tasksNumber = this.resources.tasksNumber || 2;
    this.resources.taskDefinition =
      this.resources.taskDefinition || this.resources.serviceName;
    ...
    if (!this.resources.taskRole) {
      this._taskRole = await this.generateRoleARN(
        this.resources.serviceName,
        ECS_ROLE_POLICY
      );
    } else {
  ...
  async buildDockers() {
    ...
    await this.execute(
      "docker",
      ["login", "--username", "AWS", "--password-stdin", repo],
      this.out.bind(this),
      this.out.bind(this),
      creds
    );
  ...
  _createRepository() {
    ...
    this.resources.workers.forEach(worker => {
      if (this.resources.repositoryNamespace.length > 0) {
        repositories.push(
          (this.resources.repositoryNamespace + "/" + worker).toLowerCase()
        );
      } else {
        ...
        res.repositories.forEach(repo => {
          ...
          if (idx >= 0) {
            if (this._workers[repo.repositoryName]) {
              this._workers[repo.repositoryName].repository =
                repo.repositoryUri;
            } else {
              this._workers[repo.repositoryName.split("/")[1]].repository =
                repo.repositoryUri;
            }
  ...
  _needWorkersDefinitionUpdate(oldDef) {
    ...
    for (let i in newDef) {
      ...
      for (let k in newDef[i]) {
        if (
          !oldDef.containerDefinitions[i] ||
          newDef[i][k] !== oldDef.containerDefinitions[i][k]
        ) {
          return true;
        }
      }
