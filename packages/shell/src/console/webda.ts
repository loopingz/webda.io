...
export default class WebdaConsole {
  ...
  static async serviceConfig(argv): Promise<number> {
    ...
    const service = this.webda.getService(serviceName);
    if (!service) {
      const error = "The service " + serviceName + " is missing";
...
${Object.keys(operationsExport.operations)
  ...
  static async debug(argv: yargs.Arguments) {
    ...
    const launchServe = (diagnostic: ts.Diagnostic | string) => {
      ...
      const webdaConsole = this;
      let lastLineLogLevel: WorkerLogLevel;
      const addTime = new Transform({
  ...
  static async configEncrypt(argv: yargs.Arguments): Promise<number> {
    ...
    await JSONUtils.updateFile(filename, async value => {
      if (typeof value === "string") {
        ...
        if (argv.migrate) {
          const newValue = await CryptoService.decryptConfiguration(value);
          if (value !== newValue) {
  ...
  static async deploy(argv: yargs.Arguments): Promise<number> {
    const manager = new DeploymentManager(this.app, <string>argv.deployment);
   ...
   * @returns A map of command names to their merged definitions. Each entry contains:
   *   - `description`: Help text for the command
   *   - `services`: Array of `{ name, method, type }` for each service handling this command
   *   - `args`: Merged argument definitions from all services
   *
   * @example
   * ```typescript
   * const commands = WebdaConsole.collectServiceCommands(app);
   * // {
   * //   "serve": {
   ...
   * //       { name: "MyApp/MongoMigrator", method: "migrate", type: "MyApp/MongoMigrator" }
   * //     ],
   ...
   * dependency graph). The `--service` flag filters to specific services by name
   * (supports comma-separated values and suffix matching).
   *
   * If the Webda server is not yet initialized, this method boots it before
   * dispatching. Each service's command handler is called with the parsed
   ...
   * @param argv - The parsed yargs arguments object
   * @returns `0` on success, `1` if no services match the `--service` filter
   *
   * @example
   * ```typescript
   * // Execute "migrate" on all services:
   * await WebdaConsole.executeServiceCommand("migrate", cmdInfo, { dryRun: true });
   *
   * // Execute "migrate" on a specific service:
   * await WebdaConsole.executeServiceCommand("migrate", cmdInfo, {
   *   dryRun: true,
   *   service: "PostgresMigrator"
   * });
   * ```
   */
  ...
  static async handleCommand(args, versions, output: WorkerOutput = undefined): Promise<number> {
    const res = await this.handleCommandInternal(args, versions, output);
  ...
  static loadExtensions(appPath) {
    const getAppPath = re => path.join(appPath, re);
    ...
    let files = [];
    for (const nodeModules of paths) {
    ...
    const appCustom = getAppPath("webda.shell.json");
  ...
  static async schema(argv: yargs.Arguments) {
    argv._.shift();
    const symbol = <string>argv.type;
    const filename = <string>argv.exportFile;
    const schema = this.app.getCompiler().getSchema(symbol);
    if (filename) {
  ...
  static async diagram(argv) {
    ...
    const diagram = new DiagramTypes[type]();
    if (argv.exportFile) {
  ...
  static async handleCommandInternal(args, versions, output: WorkerOutput = undefined): Promise<number> {
    ...
    if (!WebdaConsole.builtinCommands()[argv._[0]] || argv.help) {
      ...
      for (const cmd of Object.keys(this.extensions)) {
        const ext = this.extensions[cmd];
    ...
    try {
      ...
      if (this.app && this.app.getPackageWebda().logo && this.terminal) {
        const logo = this.app.getPackageWebda().logo;
  ...
  static async store(argv: yargs.Arguments): Promise<number> {
    this.webda = new WebdaServer(this.app);
    await this.webda.init();
    const store = this.webda.getService<Store>(<string>argv.storeName);
    if (!store) {
    ...
    switch (argv.action) {
      case "export":
        const filepath = <string>argv._[1] || "./export.ndjson.gz";
        ...
        const p = new Promise(resolve => fsWriter.on("finish", resolve));
        writer.end();
        await p;
    }
  }
  ...
  static async generateOpenAPI(argv: yargs.Arguments): Promise<void> {
    this.webda = new WebdaServer(this.app);
    this.webda.initStatics();
    const openapi = this.webda.exportOpenAPI(!argv.includeHidden);
    const name = <string>argv.exportFile || "./openapi.json";
