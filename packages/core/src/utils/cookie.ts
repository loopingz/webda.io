...
class SecureCookie {
  ...
  toJSON() {
    ...
    for (let prop in this) {
      /**
       * Prefixed fileds with underscore are "server-side-only" protected
       */
      if (prop[0] === "_") {
        continue;
      }
  ...
  save(ctx: Context) {
    if (this.needSave()) {
      var params = {
         ...
         * but this parameter is expected to be mililseconds (eventually received divided by 1000 by the browser)
         */
        maxAge: 86400 * 7 * 1000,
        sameSite: "Lax"
      };
