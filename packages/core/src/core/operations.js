 ...
 */
function checkOperationPermission(context, operationId, operation) {
    if (!operation) {
        throw new WebdaError.NotFound(`${operationId} Unknown`);
    }
    if (operation.permission) {
        operation.permissionQuery ?? (operation.permissionQuery = new QueryValidator(operation.permission));
 ...
 * @param operationId
 */
async function checkOperation(context, operationId) {
 ...
 */
export async function callOperation(context, operationId) {
    const operations = useInstanceStorage().operations;
 ...
 * @param definition
 */
export function registerOperation(operationId, definition) {
...
function Operation(...args) {
    const annotate = (target, context, options = {}) => {
        var _a;
        (_a = context.metadata)["webda.operations"] ?? (_a["webda.operations"] = []);
        context.metadata["webda.operations"].push({
            id: context.name,
            ...options,
