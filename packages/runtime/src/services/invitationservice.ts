...
> extends Service<T, E> {
  ...
  resolve(): this {
    ...
    const Metadata = useModelMetadata(this.model as any);
    const url = this.getParameters().url || `/${Metadata.Plural}`;
    // Register routes
    this.addRoute(`${url}/{uuid}/invitations`, ["GET", "POST", "PUT", "DELETE"], this.invite, {
      tags: [Metadata.Plural],
    ...
    for (const invit of userModel[this.parameters.mapAttribute] || []) {
      if (invit.model === model) {
        await this.authenticationService
          ...
          .ref(user)
          // @ts-ignore
          .deleteItemFromCollection(this.parameters.mapAttribute, index, "model", model);
        return;
      }
      index++;
    }
    ...
    for (const invitation of invitations) {
      ...
      if (invitation.ident) {
        if (this.parameters.autoAccept) {
          model[this.parameters.attribute][invitation.ident.getUser().toString()] = body.metadata;
        } else if (!model[this.parameters.attribute][invitation.ident.getUser().toString()]) {
          // Add to the user
          model[this.parameters.pendingAttribute][`user_${invitation.ident.getUser().toString()}`] = body.metadata;
        }
        promises.push(
          (async () => {
