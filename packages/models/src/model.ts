...
/**
 * Make only the properties in K optional, keeping the rest required.
 */
...
export abstract class Model extends RepositoryStorageClassMixIn(Object) implements Storable {
  ...
  /**
   * Internal backing store for morpher-generated accessor properties.
   * Populated automatically when getter/setter pairs are generated at build time.
   */
  protected [WEBDA_STORAGE]: Record<string, any> = {};
  /**
  ...
  /**
   * Return the string representation of the primary key
  ...
  /**
   * Get a reference to this model instance in its repository
   ...
   * @param this - the model instance
   * @param value - the primary key value to set
  ...
  /**
   * Get the repository associated with this model class
   ...
   *
   * Uses `for...in` to include both own enumerable properties (regular class fields)
   * and inherited enumerable prototype getters generated by the morpher for coercible
   * fields (e.g. Date). Symbol-keyed internals and methods are excluded automatically.
   * @returns the model instance
   */
  toJSON(): this {
    return this;
  ...
  /**
   * Helper to emit events from the class
   * @param event - the event name to emit
   * @param payload - the event data
   */
  protected async emit<T extends keyof this[typeof WEBDA_EVENTS]>(
    event: T,
    payload: this[typeof WEBDA_EVENTS][T]
  ): Promise<void> {
    await this.getRepository().emit(event, payload);
  }

  /**
   * Custom deserializers for the model
   * @returns a map of field deserializers, or undefined
   */
  ...
  /** Built-in deserializer helpers for common types */
  ...
  /**
   * Delete a model
   * @returns a promise resolving when deleted (void)
   */
  ...
  /**
   * Patch the object
   * @param data - the partial data to patch with
...
export class UuidModel extends Model {
   ...
   * Override to provide custom ID generation logic.
   * @param data - Optional partial data used during construction
