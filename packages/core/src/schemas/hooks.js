...
function getAjv() {
    if (!ajvInstance) {
        ajvInstance = new Ajv({ allErrors: true, strict: false, allowUnionTypes: true });
        // When content is generated
        ajvInstance.addKeyword("$generated");
        addFormats.default(ajvInstance);
    }
    ajvInstance.rawSchema ?? (ajvInstance.rawSchema = {});
    return ajvInstance;
}
/**
 ...
 */
export function validateModelSchema(webdaObject, object, ignoreRequired) {
 ...
 */
export function validateSchema(schema, object, ignoreRequired) {
    const ajv = getAjv();
...
export function hasSchema(webdaObject) {
    let key;
