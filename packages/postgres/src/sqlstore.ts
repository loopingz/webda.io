...
export abstract class SQLStore<
  T extends CoreModel,
  K extends SQLStoreParameters = SQLStoreParameters
> extends Store<T, K> {
  ...
  duplicateExpression(expression: WebdaQL.Expression): WebdaQL.Expression {
    if (expression instanceof WebdaQL.AndExpression) {
      return new WebdaQL.AndExpression(
        expression.children.map((exp) => this.duplicateExpression(exp))
      );
      ...
      if (expression.operator === "IN") {
        ...
        return new WebdaQL.OrExpression(
          (<string[]>expression.value).map(
            (v) => new SQLComparisonExpression("=", attr, v)
          )
        );
      }
  ...
  async find(query: WebdaQL.Query): Promise<{
    ...
    if (query.orderBy && query.orderBy.length) {
      sql +=
        ...
        query.orderBy
          .map(
            (c) =>
              `${this.mapExpressionAttribute(c.field.split("."))} ${
                c.direction
              }`
          )
          .join(", ");
    ...
    const results = (await this.sqlQuery(sql)).rows.map((c) =>
      this.initModel(c)
    );
    return {
      results,
  ...
  async _exists(uid: string): Promise<boolean> {
    let res = await this.sqlQuery(
      ...
      } WHERE ${this.getModel().getUuidField()} = '${this.getUuid(uid)}'`
    );
  ...
  async _get(uid: string, raiseIfNotFound?: boolean): Promise<T> {
    let res = await this.sqlQuery(
      `${this.getModel().getUuidField()} = '${this.getUuid(uid)}'`
    );
  ...
  async getAll(list?: string[]): Promise<T[]> {
    if (list) {
      return (
        await this.sqlQuery(
          list.map((uuid) => `uuid='${this.getUuid(uuid)}'`).join(" OR ")
        )
      ).rows;
    }
  ...
  async _update(
    ...
    let q = `UPDATE ${this.parameters.table} SET data='${object.toStoredJSON(
      true
    )}' WHERE uuid='${this.getUuid(uid)}'`;
  ...
  async _save(object: T): Promise<any> {
    await this.sqlQuery(
      `INSERT INTO ${this.parameters.table}(uuid,data) VALUES('${this.getUuid(
        object
