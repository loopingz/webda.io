...
class ProtoGeneratorTest {
  ...
  /**
   * Operation schemas that carry their own `definitions` block (e.g.
   * an output schema generated by the compiler with hoisted
   * definitions) get those folded into the flat schema registry
   * before the proto walker resolves refs. Without this the
   * `BinaryMap<T>` ref inside a method's output schema would fall
   * back to `google.protobuf.Struct` even though the definition is
   * sitting next to it in the same schema.
   */
  @test
  operationSchemaLocalDefinitionsAreFoldedIntoRegistry() {
    const schemas: Record<string, JSONSchema7> = {
      "Wrap.Output": {
        type: "object",
        properties: {
          map: { $ref: "#/definitions/BinaryMap" }
        },
        definitions: {
          BinaryMap: {
            type: "object",
    ...
    const operations = { "Wrap.Get": { output: "Wrap.Output" } };

    const result = generateProto(operations, schemas);
    assert.ok(
      result.includes("BinaryMap map = 1;"),
      "ref to a local definition resolves to a real message instead of Struct"
    );
    assert.ok(result.includes("message BinaryMap {"), "the local definition is emitted as a top-level message");
  }
