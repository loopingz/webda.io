 ...
 * Example:
 * ```js
 * stringParameter("Hello ${user}", { user: "John" }) // will return "Hello John"
 * ```
 ...
 * @returns
 */
function stringParameter(templateString, replacements = {}) {
    ...
    return new Function("return `" + (" " + templateString).replace(/([^\\])\$\{([^}{]+)}/g, "$1${this.$2}").substring(1) + "`;").call(replacements);
}
/**
 ...
 */
export function templateVariables(object, replacements = {}) {
