...
class FileStoreTest extends StoreTest {
  ...
  @test
  async cov() {
    let identStore: FileStore<CoreModel & { test: number; plops: any[] }> =
      this.getService<any>("idents");
    let userStore: FileStore<CoreModel> =
      this.getService<FileStore<CoreModel>>("users");
    ...
    assert.throws(
      () =>
        identStore.checkCollectionUpdateCondition(
          ident,
          "plops",
          undefined,
          1,
    ...
    assert.rejects(
      () =>
        identStore.simulateUpsertItemToCollection(
          undefined,
          <any>"__proto__",
          undefined,
          new Date()
    ...
    assert.throws(
      () => userStore.getParentUrl(),
      /Parent store need to be exposed/
    );
  }

  @test
  async configuration() {
    let identStore: FileStore<CoreModel> =
      this.getService<FileStore<CoreModel>>("idents");
    assert.strictEqual(
  ...
  @test("JSON Schema - Create") async schemaCreate() {
    ...
    let ctx = await this.webda.newWebContext(
      new HttpContext("webda.io", "GET", "/")
    );
    let executor = this.getExecutor(ctx, "test.webda.io", "POST", "/tasks", {
      noname: "Task #1",
    });
    this.webda.getModules().schemas["webdatest/task"] = FileUtils.load(
      "./test/schemas/task.json"
    );
