...
class InvitationTest extends WebdaTest {
  ...
  async before() {
    ...
    this.authentication =
      this.webda.getService<Authentication>("Authentication");
  ...
  async initNoTemplate() {
    let stub = sinon
      .stub(this.mailer, "hasNotification")
      .callsFake(() => false);
    try {
      this.service.getParameters().notification = "plop";
      await assert.rejects(
        () => this.service.init(),
        /Email template should exist/
      );
    } finally {
      stub.restore();
    }
  ...
  async inviteOnAclWithAutoAccept() {
    ...
    sinon
      .stub(this.service.notificationService, "sendNotification")
      .callsFake(async () => {});
    // New model as owner
    let ctx = await this.newContext();
    let user = await this.authentication
      .getUserStore()
      .save({ displayName: "Webda.io Test" });
    ...
    const ident1 = await this.authentication
      .getIdentStore()
      .get(`test1@webda.io_email`);
    const ident2 = await this.authentication
      .getIdentStore()
      .get(`test2@webda.io_email`);
    ...
    await this.execute(
      ...
      "test.webda.io",
      "POST",
      `/companies/${company.getUuid()}/invitations`,
      {
        idents: [
          "test1@webda.io_email",
          "test3@webda.io_email",
          "test4@webda.io_email",
        ],
    ...
    let userCheck = await this.authentication
      .getUserStore()
      .get(ident1.getUser());
    ...
    // Missing model
    await this.invitations.setAttribute(
      "test4@webda.io_email_invit",
      <any>"invit_test",
      "all"
    );
    ...
    const ident3 = await this.authentication
      .getIdentStore()
      .get(`test3@webda.io_email`);
    const ident4 = await this.authentication
      .getIdentStore()
      .get(`test4@webda.io_email`);
  ...
  async invitationDeletedModel() {
    ...
    let checkUser = await this.authentication
      .getUserStore()
      .get(ident1.getUser());
