...
function commandLine(line) {
  consoleService.logger = new webda.MemoryLogger(
    consoleService,
    "MemoryLogger",
    {
  ...
  describe("config", () => {
    it("exporter", () => {
      return commandLine("-d Test --noCompile config test.exports.json").then(
        () => {
          checkTestDeploymentConfig(
            JSON.parse(fs.readFileSync("test.exports.json"))
          );
        }
      );
    });
    ...
    it("exporter - bad deployment", () => {
      return commandLine("-d TestLambda config test.export.json").then(() => {
        assert.equal(
          consoleService.logger.getLogs()[0].args[0],
