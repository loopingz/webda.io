...
export class InvitationParameters extends ServiceParameters {
  ...
  constructor(params: any) {
    ...
    if (!this.attribute.startsWith("_")) {
      throw new Error(
        `Attribute '${this.attribute}' needs to start with a _ to be only modified by server`
      );
    }
    this.pendingAttribute ??= `${this.attribute}Pending`;
    if (!this.pendingAttribute.startsWith("_")) {
      throw new Error(
        `Attribute '${this.pendingAttribute}' needs to start with a _ to be only modified by server`
      );
    }
...
> extends Service<T> {
  ...
  resolve(): this {
    super.resolve();
    // Register
    this.authenticationService.on(
      "Authentication.Register",
      (evt: EventAuthenticationRegister) => this.registrationListener(evt)
    );
    const url = this.modelStore.getParameters().expose.url;
    this.addRoute(
      `${url}/{uuid}/invitations`,
      ["GET", "POST", "PUT", "DELETE"],
      this.invite
    );
    return this;
  ...
  async init(): Promise<this> {
    ...
    if (
      ...
      !(await this.notificationService.hasNotification(
        this.parameters.notification
      ))
    ) {
  ...
  async answerInvitation(
    ctx: WebContext<InvitationAnswerBody>,
    model: CoreModel
  ) {
    ...
    if (model === undefined) {
      await this.removeInvitationFromUser(
        ctx.getCurrentUserId(),
        ctx.getParameters().uuid
      );
      throw 410;
    }
    ...
    if (model[this.parameters.pendingAttribute][`user_${user.getUuid()}`]) {
      metadata =
        model[this.parameters.pendingAttribute][`user_${user.getUuid()}`];
  ...
  protected async updateModel(model: CoreModel) {
    await model.patch({
      ...
      [this.parameters.pendingAttribute]:
  ...
  async uninvite(ctx: OperationContext<Invitation>, model: CoreModel) {
    ...
    for (const ident of body.idents) {
      if (model[this.parameters.pendingAttribute][`ident_${ident}`]) {
        ...
        promises.push(
          (async () => {
            const id = await this.authenticationService
              .getIdentStore()
              .get(ident);
            if (id && id.getUser()) {
   ...
   * @param user
   */
  protected async removeInvitationFromUser(
    user: string,
    model: string
  ): Promise<void> {
      ...
      if (invit.model === model) {
        await this.authenticationService
          ...
          .deleteItemFromCollection(
            user,
            <any>this.parameters.mapAttribute,
            index,
            model,
            "model"
          );
  ...
  async invite(ctx: WebContext) {
    ...
    const invitations: { invitation: string; ident: Ident }[] =
      await Promise.all(
    ...
    for (const invitation of invitations) {
      ...
      if (invitation.ident) {
        if (this.parameters.autoAccept) {
          model[this.parameters.attribute][invitation.ident.getUser()] =
            body.metadata;
        } else if (
          !model[this.parameters.attribute][invitation.ident.getUser()]
        ) {
          // Add to the user
          model[this.parameters.pendingAttribute][
            `user_${invitation.ident.getUser()}`
          ] = body.metadata;
        }
        promises.push(
          (async () => {
            const user = await this.authenticationService
              .getUserStore()
              .get(invitation.ident.getUser());
            invitedUsers.push(user);
            await this.addInvitationToUser(
              model,
              user,
              inviter,
              metadata,
      ...
      if (await this.invitationStore.exists(invitUuid)) {
        promises.push(
          this.invitationStore.setAttribute(
            invitUuid,
            <any>this.getInvitationAttribute(model.getUuid()),
            invitInfo
          )
  ...
  async addInvitationToUser(
    ...
    if (
      (user[this.parameters.mapAttribute] || []).filter(
        (p) => p.model === model.getUuid()
      ).length
    ) {
      return;
    }
  ...
  async registrationListener(evt: EventAuthenticationRegister) {
    ...
    if (this.parameters.autoAccept) {
      for (const info of infos) {
        ...
        // Set the metadata
        info.model[this.parameters.attribute][evt.user.getUuid()] =
          info.metadata;
        // Remove pending
        delete info.model[this.parameters.pendingAttribute][
          `ident_${evt.identId}`
        ];
