...
/**
 * Focused smoke tests for PostgresStore. The harness work in @webda/core
 * (Store.computeParameters + Store.setModelDefinitionHelper) and the
 * per-model repository in PostgresStore now exist, but extending the
 * abstract StoreTest reveals routing issues where Repositories[Ident]
 * still resolves to MemoryRepository in some test runners — needs a
 * separate diagnostic pass. These smoke tests keep the build green
 * while exercising the PostgresStore lifecycle directly through SQL.
 */
@suite
