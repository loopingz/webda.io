...
class OAuthServiceTest extends WebdaTest {
  ...
  async testCheckRequest() {
    ...
    assert.strictEqual(
      await this.service.checkRequest(ctx),
      true,
      "should allow bouzouf.com referer"
    );
    ctx.setHttpContext(
      new HttpContext("bouzouf.com", "GET", "/fake", "https", 80, {
    ...
    assert.strictEqual(
      await this.service.checkRequest(ctx),
      true,
      "should allow bouzouf2.com referer"
    );
    ...
    assert.strictEqual(
      await this.service.checkRequest(ctx),
      false,
      "should not allow bouzouf12.com referer"
    );
    ...
    assert.strictEqual(
      await this.service.checkRequest(ctx),
      false,
      "should allow bouzouf.com only on its own url"
    );
    ...
    assert.strictEqual(
      await this.service.checkRequest(ctx),
      false,
      "should not allow without referer"
    );
  ...
  async misc() {
    ...
    assert.notStrictEqual(
      this.service._authenticationService,
      undefined,
      "Should get default Authentication service"
    );
    ...
    // Register with oauth
    await this.service.handleReturn(
      ctx,
      "plop",
    ...
    assert.strictEqual(
      await this.webda.getService<Store<Ident>>("Idents").exists("plop_fake"),
      true
    );
    assert.strictEqual(
      await this.webda
        .getService<Store<Ident>>("Idents")
        .exists("rcattiau@gmail.com_email"),
      true
    );
    ...
    assert.strictEqual(
      this.service._authenticationService,
      undefined,
      "Should not get any Authentication service"
    );
    await this.execute(ctx, "webda.io", "GET", "/bouzouf/scope");
    assert.deepStrictEqual(JSON.parse(<string>ctx.getResponseBody()), [
      "email",
      "image",
    ]);
    let event = 0;
    ...
    await this.getExecutor(ctx, "webda.io", "GET", "/bouzouf/callback").execute(
      ctx
    );
    assert.strictEqual(event, 2);
    assert.strictEqual(ctx.getResponseHeaders()["Location"], "bouzouf.com");
    assert.strictEqual(
      ctx.getSession<OAuthSession>().oauth.redirect,
      undefined
    );
